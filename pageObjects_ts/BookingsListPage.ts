...
export class BookingsListPage {
    ...
    getBookingCard(ref: string): Locator { // dynamic locator — runtime value, so a method, not a constructor field
    ...
    async verifyBookingCard(ref: string, title: string) {
    ...
    async openBookingDetail(ref: string) {
    ...
    async verifyBookingAbsent(ref: string) {
