...
async function createBooking(poManager, featuredIndex, quantity) {
...
test.describe('Booking management', () => {
    ...
    test('Bookings list view POM test @smoke @regression', async ({ page }) => {
        test.setTimeout(90_000);
        const poManager = new POManager(page);

        // -- Step 1: Login and create two bookings (precondition) --
        await loginAs(poManager, USER_A);
        const b1 = await createBooking(poManager, 0, 1);
        const b2 = await createBooking(poManager, 1, 1);
    ...
    test('Booking detail view POM test @regression', async ({ page }) => {
        test.setTimeout(90_000);
        const poManager = new POManager(page);

        // -- Step 1: Login and create a 2-ticket booking (precondition) --
        await loginAs(poManager, USER_A);
        const { title, ref } = await createBooking(poManager, 0, 2);
        ...
        await bookingDetailPage.verifyCustomerDetails(CUSTOMER);
        await bookingDetailPage.verifyPaymentSummary(2);
        await bookingDetailPage.verifyActionsAvailable();
    });

    // TC-005: Cancel a single booking
    test('Single booking cancellation POM test @regression', async ({ page }) => {
        test.setTimeout(90_000);
        const poManager = new POManager(page);

        // -- Step 1: Login and create a booking to cancel (precondition) --
        await loginAs(poManager, USER_A);
        const { ref } = await createBooking(poManager, 0, 1);
    ...
    test('Clear all bookings POM test @regression', async ({ page }) => {
        ...
        for (const idx of [0, 1, 2]) {
            await createBooking(poManager, idx, 1);
        }

        // -- Step 2: Bookings list shows at least three cards --
        const bookingsListPage = poManager.getBookingsListPage();
        await bookingsListPage.goto(BASE_URL);
        expect(await bookingsListPage.getCardCount()).toBeGreaterThanOrEqual(3);
    ...
    test('Static event per-user seat isolation POM test @regression', async ({ browser }) => {
        test.setTimeout(120_000);
        ...
        await loginAs(poA, USER_A);
        const { title: titleA } = await createBooking(poA, 0, 3);
